...
// Setting constants
define('EME_VERSION', '2.2.80');
define('EME_DB_VERSION', 351);
...
function eme_insertMyRewriteRules($rules) {
   ...
   foreach ($events_prefixes as $events_prefix) {
	   ...
	   $newrules['(.*/)?'.$events_prefix.'confirm/(.*)'] = 'index.php?page_id='.$events_page_id.'&eme_pmt_rndid=$matches[2]';
...
require_once("eme_attendances.php");
...
require_once("eme_countries.php") ;
require_once("eme_gdpr.php");
require_once("eme_tasks.php");
require_once("eme_translate.php");
...
function _eme_uninstall($force_drop=0) {
   ...
   if ($drop_data || $force_drop) {
      ...
      eme_drop_table(COUNTRIES_TBNAME);
      eme_drop_table(STATES_TBNAME);
      eme_drop_table(ATTENDANCES_TBNAME);
      eme_drop_table(TASKS_TBNAME);
      eme_drop_table(TASK_SIGNUPS_TBNAME);
   }
   if ($drop_settings || $force_drop) {
...
function eme_create_formfields_table($charset,$collate,$db_version) {
   ...
   if (!eme_table_exists($table_name)) {
	 ...
	 eme_drop_table($wpdb->prefix.FIELDTYPES_TBNAME);
      }
...
function eme_create_answers_table($charset,$collate,$db_version) {
   ...
   if (!eme_table_exists($table_name)) {
	 ...
	 $cf_table_name = $wpdb->prefix.MEMBERSHIPS_CF_TBNAME;
	 ...
	 $cf_table_name = $wpdb->prefix.EVENTS_CF_TBNAME;
	 if (eme_table_exists($cf_table_name)) {
	 ...
	 $cf_table_name = $wpdb->prefix.LOCATIONS_CF_TBNAME;
	 if (eme_table_exists($cf_table_name)) {
...
function eme_create_countries_table($charset,$collate,$db_version) {
   global $wpdb;
   $table_name = $wpdb->prefix.COUNTRIES_TBNAME;
...
function eme_create_states_table($charset,$collate,$db_version) {
   global $wpdb;
   $table_name = $wpdb->prefix.STATES_TBNAME;
...
function eme_create_task_tables($charset,$collate,$db_version) {
   global $wpdb;
   $table_name = $wpdb->prefix.TASKS_TBNAME;
   ...
   $table_name = $wpdb->prefix.TASK_SIGNUPS_TBNAME;
   if (!eme_table_exists($table_name)) {
      $sql = "CREATE TABLE ".$table_name." (
         id int(11) NOT NULL auto_increment,
