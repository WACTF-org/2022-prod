...
function eme_add_multibooking_form($events,$template_id_header = 0,$template_id_entry = 0,$template_id_footer = 0,$eme_register_empty_seats = 0,$is_multibooking = 1,$only_if_not_registered = 0,$only_one_event = 0, $only_one_seat=0, $simple=0) {
   global $eme_timezone;
   ...
   if (has_filter('eme_add_booking_form_prefilter')) $format_entry = apply_filters('eme_add_booking_form_prefilter',$format_entry);
   ...
   foreach ($events as $tmp_event) {
	   ...
	   if (!$seats_available) {
		   ...
		   if ($is_multibooking && $only_one_event) {
			   if ($simple) {
				   $value = eme_replace_event_placeholders($format_entry,$tmp_event);
				   $list = array($event_id => $value);
				   ...
				   $form_html .= "<option value='$event_id'>".eme_replace_event_placeholders($format_entry,$tmp_event)."</option>";
			   }
		   ...
		   if ($is_multibooking) {
			   if ($simple)
				   ...
				   $form_html .= eme_replace_rsvp_formfields_placeholders ($tmp_event,$new_booking,$format_entry,$is_multibooking);
		   } else {
...
function eme_add_simple_multibooking_form_shortcode($atts) {
   extract ( shortcode_atts ( array ('id'=>0,'recurrence_id'=>0,'category_id'=>0,'template_id_header'=>0,'template_id'=>0,'template_id_footer'=>0,'register_empty_seats'=>0,'only_if_not_registered'=>0,'only_one_event'=>0,'only_one_seat'=>0,'scope'=>'','order'=>'ASC'), $atts));
   ...
   if ($ids)
      return eme_add_multibooking_form($events,$template_id_header,$template_id,$template_id_footer,$register_empty_seats,1,$only_if_not_registered,$only_one_event,$only_one_seat,1);
}

function eme_add_multibooking_form_shortcode($atts) {
   extract ( shortcode_atts ( array ('id'=>0,'recurrence_id'=>0,'category_id'=>0,'template_id_header'=>0,'template_id'=>0,'template_id_footer'=>0,'register_empty_seats'=>0,'only_if_not_registered'=>0,'only_one_event'=>0,'only_one_seat'=>0,'scope'=>'','order'=>'ASC','simple' => 0), $atts));
   $register_empty_seats = filter_var($register_empty_seats,FILTER_VALIDATE_BOOLEAN);
   $only_if_not_registered = filter_var($only_if_not_registered,FILTER_VALIDATE_BOOLEAN);
   $only_one_event = filter_var($only_one_event,FILTER_VALIDATE_BOOLEAN);
   $only_one_seat = filter_var($only_one_seat,FILTER_VALIDATE_BOOLEAN);
   ...
   //if ($ids && $template_id_header && $template_id && $template_id_footer)
   if ($ids)
      return eme_add_multibooking_form($events,$template_id_header,$template_id,$template_id_footer,$register_empty_seats,1,$only_if_not_registered,$only_one_event,$only_one_seat,$simple);
}

function eme_booking_list_shortcode($atts) {
   extract ( shortcode_atts ( array ('id'=>0,'template_id'=>0,'template_id_header'=>0,'template_id_footer'=>0,'rsvp_status'=>0,'approval_status'=>0,'paid_status'=>0), $atts));
...
function eme_mybooking_list_shortcode($atts) {
   extract ( shortcode_atts ( array ('id'=>0,'template_id'=>0,'template_id_header'=>0,'template_id_footer'=>0,'future'=>null,'scope'=>'future','rsvp_status'=>0,'approval_status'=>0,'paid_status'=>0), $atts));
   $approval_status = intval($approval_status);
   $rsvp_status = intval($rsvp_status);
...
function eme_attendee_list_shortcode($atts) {
   extract ( shortcode_atts ( array ('id'=>0,'template_id'=>0,'template_id_header'=>0,'template_id_footer'=>0,'rsvp_status'=>0,'approval_status'=>0,'paid_status'=>0,'order'=>''), $atts));
   $approval_status = intval($approval_status);
   $rsvp_status = intval($rsvp_status);
   if ($approval_status==1)
	   $rsvp_status=EME_RSVP_STATUS_PENDING;
...
function eme_attendees_report_link_shortcode($atts) {
   global $post;
   extract ( shortcode_atts ( array ('title'=>__('Attendees CSV','events-made-easy'),'scope'=>'this_month','event_template_id'=>0,'attend_template_id'=>0,'category' => '','notcategory' => '','public_access' => 0), $atts));
   $public_access = filter_var($public_access,FILTER_VALIDATE_BOOLEAN);
   if ((!is_user_logged_in() || !current_user_can(get_option('eme_cap_list_registrations'))) && !$public_access) return;
...
function eme_bookings_report_link_shortcode($atts) {
   global $post;
   extract ( shortcode_atts ( array ('title'=>__('Bookings CSV','events-made-easy'),'event_id'=>0,'template_id'=>0,'template_id_header'=>0,'public_access' => 0), $atts));
   $public_access = filter_var($public_access,FILTER_VALIDATE_BOOLEAN);
   if ((!is_user_logged_in() || !current_user_can(get_option('eme_cap_list_registrations'))) && !$public_access) return;
   // public access? Then page needs to be password protected
   if ($public_access && empty($post->post_password))
...
function eme_cancel_bookings_form($event_id) {
   ...
   if (eme_is_event_rsvp_ended($event) || eme_is_event_cancelrsvp_ended($event)) {
	   ...
	   $form_html .= "</form></div>";
	   if (has_filter('eme_cancel_booking_form_filter')) $form_html=apply_filters('eme_cancel_booking_form_filter',$form_html);
...
function eme_add_bookings_ajax() {
	...
	if ($payment_id && eme_event_can_pay_online($event) && !$event['event_properties']['skippaymentoptions'] && !$event['event_properties']['require_user_confirmation']) {
		if ($event['event_properties']['use_captcha']) {
		...
		if ($total_price > 0) {
			...
			if ($booking['waitinglist']) {
				...
				echo json_encode(array('Result'=>'OK','keep_form'=>0,'htmlmessage'=>$form_result_message,'paymentform'=>$payment_form));
				...
				echo json_encode(array('Result'=>'OK','keep_form'=>0,'htmlmessage'=>$form_result_message,'waitperiod'=>$waitperiod,'paymentredirect'=>$payment_url));
			} else {
				$payment_form = eme_payment_form($payment_id);
				echo json_encode(array('Result'=>'OK','keep_form'=>0,'htmlmessage'=>$form_result_message,'paymentform'=>$payment_form));
			}
...
function eme_cancel_bookings_ajax() {
   ...
   echo json_encode(array('Result'=>'OK','htmlmessage'=>$form_html));
   wp_die();
}
...
function eme_multibook_seats($events, $send_mail, $format, $is_multibooking=1, $simple=0) {
   ...
   if (!eme_is_empty_string($event['event_properties']['rsvp_password']) && !$eme_is_admin_request) {
	   if (!isset( $_POST['rsvp_password'] )) {
		   ...
		   return array(0=>$form_html,1=>$booking_ids);
   ...
   foreach ($events as $event) {
	 ...
	 if (!$is_multibooking && !eme_is_multi($min_allowed) && $min_allowed>0) {
      ...
      if (is_user_logged_in()) {
	      ...
	      $current_userid=get_current_user_id();
	      if (!$eme_is_admin_request && (current_user_can( get_option('eme_cap_edit_events')) ||
		      (current_user_can( get_option('eme_cap_author_event')) && ($event['event_author']==$current_userid || $event['event_contactperson_id']==$current_userid)))) {
      ...
      if (!$eme_is_admin_request) {
	      ...
	      if (!eme_is_email($bookerEmail,1)) {
		      $form_html .= __('Please enter a valid mail address','events-made-easy');
		      continue;
	      }
	      ...
	      if (get_option('eme_rsvp_check_required_fields')) {
		      ...
		      foreach ($all_required_fields as $required_field) {
			      if (preg_match ("/LASTNAME|EMAIL|SEATS|SPACES|DISCOUNT|PASSWORD/",$required_field)) {
				      ...
				      if (eme_is_empty_string($_POST['phone'])) array_push($missing_required_fields, __('Phone number','events-made-easy'));
			      ...
			      } elseif (preg_match ("/(ADDRESS1|ADDRESS2|CITY|ZIP|BIRTHDATE|BIRTHPLACE)/",$required_field, $matches)) {
				      $fieldname=strtolower($matches[1]);
				      if (eme_is_empty_string($_POST[$fieldname]) && $fieldname=='address1') array_push($missing_required_fields, __('Address1','events-made-easy'));
				      if (eme_is_empty_string($_POST[$fieldname]) && $fieldname=='address2') array_push($missing_required_fields, __('Address2','events-made-easy'));
				      if (eme_is_empty_string($_POST[$fieldname]) && $fieldname=='city') array_push($missing_required_fields, __('City','events-made-easy'));
				      if (eme_is_empty_string($_POST[$fieldname]) && $fieldname=='zip') array_push($missing_required_fields, __('Zip','events-made-easy'));
				      if (eme_is_empty_string($_POST[$fieldname]) && $fieldname=='birthdate') array_push($missing_required_fields, __('Day of birth','events-made-easy'));
				      if (eme_is_empty_string($_POST[$fieldname]) && $fieldname=='birthplace') array_push($missing_required_fields, __('Place of birth','events-made-easy'));
			      } elseif (preg_match ("/STATE/",$required_field, $matches)) {
				      $fieldname="state_code";
				      if (eme_is_empty_string($_POST[$fieldname])) array_push($missing_required_fields, __('State','events-made-easy'));
			      } elseif (preg_match ("/COUNTRY/",$required_field, $matches)) {
				      $fieldname="country_code";
				      if (eme_is_empty_string($_POST[$fieldname])) array_push($missing_required_fields, __('Country','events-made-easy'));
			      } elseif (preg_match ("/COMMENT/",$required_field)) {
				      if (eme_is_empty_string($tmp_booking['booking_comment'])) array_push($missing_required_fields, __('Comment','events-made-easy'));
   ...
   $res = array(0=>$form_html,1=>$payment_id);
   return $res;
}
...
function eme_store_booking_answers($booking, $do_update=1) {
	global $wpdb;
	$fields_seen=array();
	if (empty($booking['booking_id'])) $do_update=0;
...
function eme_userconfirm_bookings($booking_ids_arr,$price) {
	foreach ($booking_ids_arr as $booking_id) {
		$booking=eme_get_booking($booking_id);
		$event=eme_get_event($booking['event_id']);
		if ($price==0 || !$event['registration_requires_approval'] || $event['event_properties']['auto_approve']) {
			$res = eme_approve_booking($booking['booking_id']);
			if ($res) {
				// booking has changed, get it again
				$booking = eme_get_booking($booking['booking_id']);
				eme_email_booking_action($booking,"approveBooking");
			}
			eme_manage_waitinglist($event);
		} else {
			$res = eme_mark_booking_pending($booking['booking_id']);
			if ($res) {
				// booking has changed, get it again
				$booking = eme_get_booking($booking['booking_id']);
...
function eme_get_available_seats($event_id,$exclude_waiting_list=0) {
	$event = eme_get_event($event_id);
	if (empty($event)) return 0;
	if (eme_is_multi($event['event_seats']))
		return array_sum(eme_get_available_multiseats($event_id,$exclude_waiting_list));
	...
	if ($event['event_properties']['ignore_pending'] == 1) {
		...
		if (eme_event_can_pay_online($event))
			$available_seats -= eme_get_young_pending_seats($event_id);
	} else {
...
function eme_get_available_multiseats($event_id,$exclude_waiting_list=0) {
	$event = eme_get_event($event_id);
	if (empty($event)) return 0;
	...
	if ($event['event_properties']['ignore_pending'] == 1) {
		...
		if (eme_event_can_pay_online($event))
			$young_pending_multiseats=eme_get_young_pending_multiseats($event_id);
		else
			$young_pending_multiseats=array();
	} else {
...
function eme_get_young_pending_seats($event_id) {
	global $eme_timezone;
	$eme_date_obj_now=new ExpressiveDate("now",$eme_timezone);
	$old_date = $eme_date_obj_now->minusMinutes(5)->getDateTime();
	return eme_get_pending_seats($event_id,$old_date);
}

function eme_get_pending_seats($event_id,$old_date="") {
	global $wpdb; 
	if (eme_is_event_multiseats($event_id))
	...
	$sql = $wpdb->prepare("SELECT COALESCE(SUM(booking_seats),0) AS booked_seats FROM $bookings_table WHERE status IN (%d,%d) AND event_id = %d $younger_than",EME_RSVP_STATUS_PENDING,EME_RSVP_STATUS_USERPENDING,$event_id);
	return $wpdb->get_var($sql);
}
...
function eme_get_young_pending_multiseats($event_id) {
	global $eme_timezone;
	$eme_date_obj_now=new ExpressiveDate("now",$eme_timezone);
	$old_date = $eme_date_obj_now->minusMinutes(5)->getDateTime();
	return eme_get_pending_multiseats($event_id,$old_date);
}
function eme_get_pending_multiseats($event_id,$old_date="") {
	global $wpdb; 
	$bookings_table = $wpdb->prefix.BOOKINGS_TBNAME;
	if (empty($old_date))
		$younger_than="";
		...
		$younger_than="AND creation_date > '$old_date'";
	$sql = $wpdb->prepare("SELECT booking_seats_mp FROM $bookings_table WHERE status IN (%d,%d) AND event_id = %d $younger_than",EME_RSVP_STATUS_PENDING,EME_RSVP_STATUS_USERPENDING,$event_id);
	$booking_seats_mp = $wpdb->get_col($sql);
	$result=array();
	foreach($booking_seats_mp as $booked_seats) {
		$multiseats = eme_convert_multi2array($booked_seats);
...
function eme_are_seats_available_for($event_id, $seats, $exclude_waiting_list=0) {
	$total_seats = eme_get_total_seats($event_id);
	if ($total_seats>0) {
		$available_seats = eme_get_available_seats($event_id, $exclude_waiting_list);
...
function eme_are_multiseats_available_for($event_id, $multiseats, $exclude_waiting_list=0) {
	$available_seats = eme_get_available_multiseats($event_id, $exclude_waiting_list);
...
function eme_get_attendees_list($event,$template_id=0,$template_id_header=0,$template_id_footer=0,$rsvp_status=0,$paid_status=0,$order="") {
	...
	if ($attendee_ids) {
		...
		$res="<p class='eme_no_bookings'>".__('No bookings made yet.','events-made-easy')."</p>";
	}
	return $res;
}
...
function eme_replace_booking_placeholders($format, $event, $booking, $is_multibooking=0, $target="html",$lang='', $take_answers_from_post=0) {
	...
	if ($booking['person_id'] == -1) {
		...
		$dyn_answers = (isset($event['event_properties']['rsvp_dyndata'])) ? eme_get_dyndata_booking_answers($booking['booking_id']) : array();
	...
	foreach($placeholders[0] as $orig_result) {
		...
		if (preg_match('/#_(RESP)?COMMENT/', $result)) {
			...
			$replacement = eme_transfer_nbr_be97_formatted($booking['transfer_nbr_be97']);
			...
			} elseif (!$booking['waitinglist'] && $payment && eme_event_can_pay_online($event)) {
			...
			$formfield = eme_get_formfield($field_key);
			if (!empty($formfield) && in_array($formfield['field_purpose'],array('generic','rsvp'))) {
...
function eme_email_booking_action($booking,$action, $is_multibooking=0) {
	...
	// here "html" is enough
	if ($mail_text_html=="htmlmail") $mail_text_html="html";
	...
	if (!empty($contact_subject) && !empty($contact_body)) {
		$mail_res = eme_queue_mail($contact_subject, $contact_body, $contact_email,$contact_name, $contact_email, $contact_name);
	}
	if (!empty($person_subject) && !empty($person_body)) {
		...
		if ($booking['status']==EME_RSVP_STATUS_USERPENDING)
			$mail_res = eme_queue_fastmail($person_subject,$person_body, $person['email'], $person_name, $contact_email, $contact_name, 0,$booking['person_id'],0, $attachment_ids_arr);
		else
			$mail_res = eme_queue_mail($person_subject,$person_body, $person['email'], $person_name, $contact_email, $contact_name, 0,$booking['person_id'],0, $attachment_ids_arr);
	}
	return $mail_res;
}
...
function eme_registration_seats_page($pending=0) {
	...
	if (isset($_GET['eme_admin_action']) && $_GET['eme_admin_action'] == "newBooking" && isset($_GET['event_id'])) {
		...
		$ret_string.= "</table>";
		if (empty($event['event_id'])) {
			$ret_string.= "<br /><img style='vertical-align: middle;' src='".$eme_plugin_url."images/warning.png' alt='warning' />".__('Warning: the event related to this booking has been removed, editing is no longer allowed!','events-made-easy')."<br />";
		} elseif ($booking['event_price'] != $event['price']) {
			$ret_string.= "<br /><img style='vertical-align: middle;' src='".$eme_plugin_url."images/warning.png' alt='warning' />".__('Warning: the price of the event has changed compared to this booking, the new price will be taken into account for changes!','events-made-easy')."<br />";
		}
			...
			if (eme_is_multi($booking['event_price'])) {
				...
				foreach($_POST['bookings'][$event_id] as $key=>$value) {
					if (preg_match('/bookedSeats(\d+)/', $key, $matches)) {
						...
						if (!eme_is_multi($event['event_seats'])) {
							$bookedSeats += $bookedSeats_mp[$field_id];
						} elseif (eme_is_multi($event['event_seats']) && $bookedSeats_mp[$field_id] > $available_multiseats[$field_id] + $already_booked_seats_mp[$field_id]) {
							$enough_seats=0;
						}
					}
				}
...
<h1><?php 
	...
	if (isset($_GET['event_id'])) {
		...
		$event=eme_get_event($event_id);
		if (empty($event)) {
				_e('No such event','events-made-easy');
				return;
		} else {
		...
		$event_id=0;
		if ($pending) 
			_e ('Manage pending bookings','events-made-easy');
		else
			_e ('Manage approved bookings','events-made-easy');
	}
?>
</h1>
   <?php if ($trash) { ?>
      <a href="<?php echo admin_url("admin.php?page=$plugin_page&trash=0$event_q_string"); ?>"><?php _e('Show regular content','events-made-easy');?></a><br />
   <?php } else { ?>
      <a href="<?php echo admin_url("admin.php?page=$plugin_page&trash=1$event_q_string"); ?>"><?php _e('Show trash content','events-made-easy');?></a><br />
   <?php } ?>
...
<?php endif; ?>
   <input type="text" name="search_customfields" id="search_customfields" placeholder="<?php _e('Filter on custom field answer','events-made-easy'); ?>" size=20 />
   <input type="text" name="search_unique" id="search_unique" placeholder="<?php _e('Filter on unique nbr','events-made-easy'); ?>" size=15 />
   <input type="text" name="search_paymentid" id="search_paymentid" placeholder="<?php _e('Filter on payment id','events-made-easy'); ?>" size=15 />
   <input type="text" name="search_pg_pid" id="search_pg_pid" placeholder="<?php _e('Filter on payment GW id','events-made-easy'); ?>" size=15 />
   </div>
   <button id="BookingsLoadRecordsButton" class="button-secondary action"><?php _e('Filter bookings','events-made-easy'); ?></button>
   </form>
   </div>
   <div>
...
<?php if ($trash) { ?>
   <option value="deleteBooking"><?php _e ( 'Permanently delete booking','events-made-easy'); ?></option>
   <option value="userConfirmBooking"><?php _e ( 'Restore booking and mark pending and awaiting user confirmation','events-made-easy'); ?></option>
   <option value="pendingBooking"><?php _e ( 'Restore booking and mark pending','events-made-easy'); ?></option>
   <option value="approveBooking"><?php _e ( 'Restore booking and mark approved','events-made-easy'); ?></option>
<?php } elseif ($pending) { ?>
   <option value="approveBooking"><?php _e ( 'Approve booking','events-made-easy'); ?></option>
   <option value="trashBooking"><?php _e ( 'Delete booking (move to trash)','events-made-easy'); ?></option>
   <?php if ($mailing_pending) { ?>
       <option value="resendPendingBooking"><?php _e ( 'Resend the mail for pending booking','events-made-easy'); ?></option>
   <?php } ?>
   ...
   <option value="partialPayment"><?php _e ( 'Partial payment','events-made-easy'); ?></option>
   <option value="unsetwaitinglistBooking"><?php _e ( 'Move booking off the waitinglist','events-made-easy'); ?></option>
   <option value="setwaitinglistBooking"><?php _e ( 'Put booking on the waitinglist','events-made-easy'); ?></option>
   <option value="sendMails"><?php _e ( 'Send generic email to selected persons','events-made-easy'); ?></option>
   <option value="rsvpMails"><?php _e ( 'Send booking related email to selected bookings','events-made-easy'); ?></option>
   <option value="pdf"><?php _e ( 'PDF output','events-made-easy'); ?></option>
   <option value="html"><?php _e ( 'HTML output','events-made-easy'); ?></option>
<?php } else { ?>
   <option value="pendingBooking"><?php _e ( 'Make booking pending','events-made-easy'); ?></option>
   <option value="trashBooking"><?php _e ( 'Delete booking (move to trash)','events-made-easy'); ?></option>
   <?php if ($mailing_approved) { ?>
       <option value="resendApprovedBooking"><?php _e ( 'Resend the mail for approved booking','events-made-easy'); ?></option>
   <?php } ?>
   <option value="markPaid"><?php _e ( 'Mark paid','events-made-easy'); ?></option>
   <option value="markUnpaid"><?php _e ( 'Mark unpaid','events-made-easy'); ?></option>
   <option value="partialPayment"><?php _e ( 'Partial payment','events-made-easy'); ?></option>
...
<?php } ?>
   ...
   <?php _e('Refund if possible?','events-made-easy'); echo eme_ui_select_binary(0,"refund"); ?>
   </span>
   <?php endif; ?>
   <span id="span_partialpayment" class="eme-hidden">
   <?php _e('Partial payment amount','events-made-easy'); echo eme_ui_number(0,"partial_amount"); ?>
   </span>
...
function eme_ajax_bookings_list() {
	...
	$search_unique = isset($_REQUEST['search_unique']) ? eme_str_numbers_only($_REQUEST['search_unique']): '';
	$search_paymentid = isset($_REQUEST['search_paymentid']) ? intval($_REQUEST['search_paymentid']): 0;
	$search_pg_pid = isset($_REQUEST['search_pg_pid']) ? esc_sql($_REQUEST['search_pg_pid']): '';
	...
	$category = isset($_REQUEST['category']) ? esc_sql($_REQUEST['category']) : '';
	...
	foreach ($bookings as $booking) {
		...
		if ($trash)
			...
			$line['edit_link'] = "<a href='".wp_nonce_url(admin_url("admin.php?page=$page&amp;eme_admin_action=editBooking&amp;booking_id=".$booking ['booking_id']),"eme_admin ".$booking ['booking_id'],'eme_admin_nonce')."' title='".__('Click here to see and/or edit the details of the booking.','events-made-easy')."'>"."<img src='".$eme_plugin_url."images/edit.png' alt='".__('Edit','events-made-easy')."'> "."</a>";
		...
		if (isset($event['event_id']) && $event['event_rsvp']) {
			if ($add_event_info)
				$event_name_info[$booking_event_id] .= "<br />".__('RSVP Info: ','events-made-easy');
			if ($add_event_info) {
				$booked_seats = eme_get_approved_seats($event['event_id']);
				$pending_seats = eme_get_pending_seats($event['event_id']);
				$booked_string = __('Approved:','events-made-easy');
				$total_seats = eme_get_total($event['event_seats']);
				if (eme_is_multi($event['event_seats'])) {
					if ($pending_seats>0)
				...
				if ($total_seats>0) {
					...
					$event_name_info[$booking_event_id] .= __('Free: ','events-made-easy').$available_seats_string;
			...
			$line['rsvp'] = "<a href='".wp_nonce_url(admin_url("admin.php?page=$page&amp;eme_admin_action=newBooking&amp;event_id=".$event['event_id']),"eme_admin ".$event['event_id'],'eme_admin_nonce')."' title='".__('Add booking for this event','events-made-easy')."'>".__('RSVP','events-made-easy')."</a>";
			if (!empty($event['event_properties']['rsvp_password'])) {
				$line['rsvp'] .= "<br />(".__('Password protected','events-made-easy').")";
			}
		} else {
			$line['rsvp'] = "";
		}
		...
		if ($booking['status']==EME_RSVP_STATUS_USERPENDING) {
			$line['seats'].='<br />'. __('(Awaiting user confirmation)','events-made-easy');
		}
		if ($booking['reminder']>0) {
		...
		$files = eme_get_uploaded_files($booking['booking_id'],"bookings");
			...
			if (isset($line[$key])) {
				$line[$key].=eme_get_uploaded_file_html($file,0,1);
			} else {
				$line[$key]=eme_get_uploaded_file_html($file,0,1);
			}
                }
		// the next is possible, but currently we don't use people answer fields in rsvp overview, so let's not do this yet
	...
	if (isset($_POST['do_action'])) {
		...
		if (eme_array_integers($ids_arr)) {
			...
			case 'trashBooking':
				...
				break;
			case 'partialPayment':
				$amount=(isset($_POST['partial_amount'])) ? $_POST['partial_amount'] : 0;
...
function eme_ajax_action_rsvp_paidandapprove($ids_arr) {
	...
	foreach ($ids_arr as $booking_id) {
		$booking = eme_get_booking ($booking_id);
		if ($booking['booking_paid'] && $booking['status']==EME_RSVP_STATUS_APPROVED) continue;
...
function eme_ajax_action_booking_partial_payment($booking_id,$amount,$send_mail) {
   ...
   if ($booking['booking_paid']) return;
...
function eme_ajax_action_mark_booking_paid($ids_arr,$action,$send_mail) {
   ...
   foreach ($ids_arr as $booking_id) {
       $booking = eme_get_booking ($booking_id);
       if ($booking['booking_paid']) continue;
       $event=eme_get_event($booking['event_id']);
       if (!empty($event)) {
...
function eme_ajax_action_send_booking_mails($ids_arr,$subject_template_id, $body_template_id) {
   ...
   foreach ($ids_arr as $booking_id) {
       ...
       if (!empty($event) && $person && is_array($person)) {
	       ...
	       $person_name=eme_format_full_name($person['firstname'],$person['lastname']);
	       $mail_res = eme_queue_mail($tmp_subject,$tmp_message, $person['email'], $person_name, $contact_email, $contact_name, 0, $person['person_id']);
...
";
	...
	// unlink old pdf
	array_map('unlink', glob("$targetPath/ticket-$template_id-*.pdf"));
	// now put new one
	$rand_id=eme_random_id();
	$target_file=$targetPath."/ticket-$template_id-$rand_id.pdf";
