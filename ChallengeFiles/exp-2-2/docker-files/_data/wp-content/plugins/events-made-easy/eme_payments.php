...
function eme_payment_gateways() {
	...
	if (has_filter('eme_payment_gateways')) $pgs=apply_filters('eme_payment_gateways',$pgs);
	return $pgs;
}
...
function eme_payment_form($payment_id,$resultcode=0,$standalone=0) {
   ...
   if ($booking['status']==EME_RSVP_STATUS_USERPENDING) {
	   ...
	   eme_userconfirm_bookings($booking_ids,$total_price);
   ...
   foreach ($bookings as $booking) {
	   ...
	   if ($total_seats == 0) {
		   ...
		   // we check for available seats, excluding waiting list
		   $seats_available = eme_are_multiseats_available_for($event_id, $bookedSeats_mp,1);
	   } else {
		   // we check for available seats, excluding waiting list
		   $seats_available = eme_are_seats_available_for($event_id, $booking['booking_seats'],1);
	   }
	   if (!$seats_available) {
...
function eme_payment_gateway_total($price,$cur,$gateway) {
   ...
   if (in_array($cur,$eme_zero_decimal_currencies_arr)) {
      ...
      if ($gateway == "payconiq") $price *=100;
      if ($gateway == "fondy") $price *= 100;
   }
   return $price;
}
...
function eme_braintree_form($item_name,$payment,$baseprice,$cur,$multi_booking=0) {
   ...
   require_once('payment_gateways/braintree/braintree_php-6.8.0/lib/Braintree.php');
   $braintree_gateway = new Braintree\Gateway(array(
...
function eme_instamojo_form($item_name,$payment,$baseprice,$cur,$multi_booking=0) {
   ...
   if (!$instamojo_key || !$instamojo_auth_token) return;
...
function eme_paypal_form($item_name,$payment,$baseprice,$cur,$multi_booking=0) {
   ...
   if (!$eme_paypal_clientid || !$eme_paypal_secret) return;
...
function eme_mercadopago_form($item_name,$payment,$baseprice,$cur,$multi_booking=0) {
   ...
   if (!$eme_mercadopago_access_token) return;
   $payment_id=$payment['id'];
   $events_page_link = eme_get_events_page();
   $success_link = eme_payment_return_url($payment,0);
   $fail_link = eme_payment_return_url($payment,1);
   ...
   require_once 'payment_gateways/mercadopago/2.4.5/vendor/autoload.php';
...
function eme_complete_instamojo_transaction($payment) {
   $instamojo_key = get_option('eme_instamojo_key');
   $instamojo_auth_token = get_option('eme_instamojo_auth_token');
   require_once 'payment_gateways/instamojo/autoload.php';
...
function eme_complete_stripe_transaction($payment) {
   require_once('payment_gateways/stripe/stripe-php-7.121.0/init.php');
...
function eme_complete_fondy_transaction($payment) {
  ...
  if (!$eme_fondy_merchant_id) return "No merchant ID";
  $eme_fondy_secret_key = get_option('eme_fondy_secret_key');
  if (!$eme_fondy_secret_key) return "No secret key";
...
function eme_notification_fdgg() {
   ...
   if ($payment['member_id']>0) {
      ...
      if ($booking_ids) {
	      ...
	      $event=eme_get_event($booking['event_id']);
	      if (empty($event)) {
		      _e('No such event','events-made-easy');return;
	      } else {
		      $cur=$event['currency'];
...
function eme_charge_braintree() {
   ...
   $result = $braintree_gateway->transaction()->sale(array(
			   "amount" => $price,
...
function eme_charge_instamojo() {
   ...
   try {
	   $instamojo_payment = $api->paymentRequestCreate(array(
		   "purpose" => $description,
		   "amount" => "$price",
...
function eme_refund_booking_paypal($booking) {
   if ($booking['pg']!="paypal" || empty($booking['pg_pid'])) return;
   require_once "payment_gateways/paypal/vendor/autoload.php";
...
function eme_refund_booking_mercadopago($booking) {
   if ($booking['pg']!="mercadopago" || empty($booking['pg_pid'])) return;
   if (get_option('eme_mercadopago_demo')==1) {
	   $eme_mercadopago_access_token=get_option('eme_mercadopago_sandbox_token');
   } else {
	   $eme_mercadopago_access_token=get_option('eme_mercadopago_live_token');
...
function eme_refund_booking_instamojo($booking) {
   if ($booking['pg']!="instamojo" || empty($booking['pg_pid'])) return;
   $instamojo_key = get_option('eme_instamojo_key');
   $instamojo_auth_token = get_option('eme_instamojo_auth_token');
   if (!$instamojo_key || !$instamojo_auth_token) return;
...
function eme_refund_booking_fondy($booking) {
   ...
   if ($booking['pg'] != $gateway || empty($booking['pg_pid'])) return;
...
function eme_charge_mollie() {
   ...
   if (!class_exists('Mollie\Api\MollieApiClient')) {
	   require_once 'payment_gateways/Mollie/2.41.0/vendor/autoload.php';
   }
   ...
   try {
      ...
      $mollie_payment = $mollie->payments->create(
            array(
               'amount'      => array("currency"=>$cur, "value"=>sprintf("%01.2f",$price)),
               'description' => $description,
...
function eme_charge_payconiq() {
   ...
   if (!class_exists('Payconiq\Client')) {
	   require_once 'payment_gateways/payconiq/2.0.1-djairhogeuens/src/Client.php';
   }
...
function eme_notification_payconiq($payconiq_paymentid=0) {
   ...
   $merchant_id = get_option("eme_${gateway}_merchant_id");
   if (!$api_key) return;
   // if no payment id is provided, it is a real notification from payconiq, so then we check the merchant id too
   if (!$payconiq_paymentid) {
	   $payload = @file_get_contents('php://input');
	   $data = json_decode($payload);
	   $payconiq_paymentid = $data->paymentId;
	   $payconiq_merchantid = $data->creditor->merchantId;
	   if ($payconiq_merchantid != $merchant_id) return;
   }
   ...
   if ($payconiq_payment->status == "SUCCEEDED" && $payconiq_payment->totalAmount*100 >= $payment_price ) {
      eme_mark_payment_paid($payment_id,1,$gateway,$payconiq_paymentid);
   }
}

function eme_refund_booking_mollie($booking) {
   if ($booking['pg']!="mollie" || empty($booking['pg_pid'])) return;
   $api_key = get_option('eme_mollie_api_key');
   if (!$api_key) return;
   // Avoid loading the Mollie API if it is already loaded by another plugin
   if (!class_exists('Mollie\Api\MollieApiClient')) {
...
function eme_update_payment_pg_pid($payment_id, $pg_pid="") {
   global $wpdb;
   $table = $wpdb->prefix.PAYMENTS_TBNAME;
...
function eme_mark_payment_paid($payment_id, $is_ipn=1, $pg="", $pg_pid="") {
   ...
   if (isset($payment['member_id']) && $payment['member_id']>0) {
      eme_accept_member_payment($payment['member_id'], $pg, $pg_pid);
      if ($is_ipn) {
	  $member = eme_get_member($payment['member_id']);
...
function eme_replace_payment_gateway_placeholders($format, $pg,$total_price,$currency,$vat_pct,$target,$lang,$do_shortcode=1) {
   ...
   $needle_offset=0;
   preg_match_all("/#_?[A-Za-z0-9_]+/", $format, $placeholders,PREG_OFFSET_CAPTURE);
...
function eme_refund_booking($booking) {
	if (empty($booking['pg']) || empty($booking['pg_pid'])) return;
	$pg=$booking['pg'];
...
function eme_cancel_payment_form($payment_randomid) {
	...
	if (!$booking_ids) {
		return "<div id='eme-cancel-payment-message-error' class='eme-message-error eme-cancel-payment eme-cancel-payment-error'>".__('Nothing found!','events-made-easy')."</div>";
	}
...
function eme_cancel_payment_ajax() {
   ...
   } elseif (get_option('eme_recaptcha_for_forms')) {
   ...
   $booking_ids=eme_get_randompayment_booking_ids($payment_randomid);
   if (!$booking_ids) {
      $form_html =  __('Nothing found!','events-made-easy');
      echo json_encode(array('Result'=>'NOK','htmlmessage'=>$form_html));
      wp_die();
   }
   // we need to know who did the booking, so get the first bookings person_id
