 ...
 *
 * @package exs/font-loader
 * @license https://opensource.org/licenses/MIT
 *
 * @sicne 1.9.9
 */

if ( ! class_exists( 'ExS_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class ExS_WebFont_Loader {
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'exs_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'exs_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'exs_get_local_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'exs_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function exs_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new ExS_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'exs_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function exs_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new ExS_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
