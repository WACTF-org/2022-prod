...
function eme_gdpr_approve_url($email) {
   ...
   $the_link = add_query_arg( array( 'eme_gdpr_approve' => $email, 'eme_gdpr_nonce'=>$nonce ), $the_link );
   if (!empty($language))
...
function eme_gdpr_ajax() {
      ...
      if (eme_count_persons_by_email($email)>0) {
	      ...
	      eme_queue_fastmail($gdpr_subject,$gdpr_body, $email, '', $contact_email, $contact_name);
      }
...
function eme_rpi_shortcode($atts) {
	...
	<div id='eme-gdpr-message-ok-$form_id' class='eme-message-success eme-gdpr-message eme-gdpr-message-success eme-hidden'></div><div id='eme-gdpr-message-error-$form_id' class='eme-message-error eme-gdpr-message eme-gdpr-message-error eme-hidden'></div><div id='div_eme-gdpr-form-$form_id' style='display: none' class='eme-showifjs'><form id='$form_id' name='eme-gdpr-form' method='post' action='#'>
		...
		<input type='email' name='eme_email' required='required' value='".$email."' placeholder='".__('E-mail','events-made-easy')."' />
		<img id='loading_gif' alt='loading' src='".$eme_plugin_url."images/spinner.gif' style='display:none;'><br />
...
#      }
      ...
      if (!empty($person_ids)) {
	      ...
	      eme_queue_fastmail($change_subject,$change_body, $email, $first_person_name, $contact_email, $contact_name);
      }
...
function eme_cpi_shortcode($atts) {
	...
	$form_html = "<noscript><div class='eme-noscriptmsg'>".__('Javascript is required for this form to work properly','events-made-easy')."</div></noscript>
		...
		$nonce
		<span id='honeypot_check'><input type='text' name='honeypot_check' value='' autocomplete='off' /></span>
		<input type='email' name='eme_email' value='".$email."' placeholder='".__('E-mail','events-made-easy')."' />
		<img id='loading_gif' alt='loading' src='".$eme_plugin_url."images/spinner.gif' style='display:none;'><br />
...
function eme_show_personal_info($email) {
	...
	if (eme_count_persons_by_email($email)>0) {
		...
		foreach ($person_ids as $person_id) {
			...
			$output .= '<tr><td>'.__('E-mail', 'events-made-easy').'</td><td>'.eme_esc_html($person['email']).'</td></tr>';
			$output .= '<tr><td>'.__('Address1', 'events-made-easy').'</td><td>'.eme_esc_html($person['address1']).'</td></tr>';
			$output .= '<tr><td>'.__('Address2', 'events-made-easy').'</td><td>'.eme_esc_html($person['address2']).'</td></tr>';
			$output .= '<tr><td>'.__('City', 'events-made-easy').'</td><td>'.eme_esc_html($person['city']).'</td></tr>';
			$output .= '<tr><td>'.__('Zip', 'events-made-easy').'</td><td>'.eme_esc_html($person['zip']).'</td></tr>';
...
function eme_gdpr_user_data_exporter( $email, $page = 1 ) {
	...
	if (eme_count_persons_by_email($email)>0) {
		...
		foreach ($person_ids as $person_id) {
			...
			$data[] = array(
				'name'  => __('E-mail', 'events-made-easy'),
				'value' => $person['email']
			);
			$data[] = array(
				'name'  => __('Address1', 'events-made-easy'),
				'value' => $person['address1']
			);
			$data[] = array(
				'name'  => __('Address2', 'events-made-easy'),
