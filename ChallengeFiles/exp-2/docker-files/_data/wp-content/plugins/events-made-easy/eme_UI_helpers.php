...
function eme_options_input_text($title, $name, $description, $type="text", $option_value=false) {
   ...
   <tr style='vertical-align:top' id='<?php echo $name;?>_row'>
      <th scope="row"><label for='<?php echo $name; ?>'><?php _e($title,'events-made-easy'); ?></label></th>
       <td>
...
?>
         ...
         <th scope="row"><label for='<?php echo $name; ?>'><?php _e($title,'events-made-easy'); ?></label></th>
         <td>
		<?php 	echo eme_ui_select($option_value, $name, $list);
...
?>
         ...
         <th scope="row"><label for='<?php echo $name; ?>'><?php _e($title,'events-made-easy'); ?></label></th>
         <td>
		<?php 	echo eme_ui_multiselect($option_value_arr, $name, $list,5,'',0,$class);
			if (!empty($description)) echo "<br />".$description;
		?>
         </td>
         </tr>
...
function eme_ui_select($option_value, $name, $list, $add_empty_first='', $required=0,$class="", $extra_attributes='') {
   ...
   foreach($list as $key => $value) {
      ...
      if (empty($t_value) && $t_value!=="0") $t_value='&nbsp;';
...
function eme_ui_multiselect_key_value($option_value, $name, $list, $key, $value, $size=3, $add_empty_first='', $required=0, $class='', $extra_attributes='') {
   // make sure it is an array, otherwise just go back
   if (!is_array($list)) return;
...
function eme_ui_checkbox_binary($option_value, $name, $label='', $required=0, $class='', $extra_attributes='') {
   ...
   if (!empty($label)) $val.= "<label for='${name}'>".eme_esc_html($label)."</label>";
   return $val;
}
