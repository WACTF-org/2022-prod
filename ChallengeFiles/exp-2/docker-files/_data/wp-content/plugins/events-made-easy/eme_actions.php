   ...
   if (isset($_POST['eme_ajax_action']) && $_POST['eme_ajax_action'] == 'task_autocomplete_people' && isset($_POST['task_lastname'])) {
	 ...
	 if (!isset($_POST['eme_frontend_nonce']) ||
	     (isset($_POST['eme_frontend_nonce']) && !wp_verify_nonce( $_POST['eme_frontend_nonce'], 'eme_frontend'))) {
	      ...
	      echo json_encode(__('Access denied!','events-made-easy'));
	 }
...
#   wp_register_script( 'eme-jquery-datatables', $eme_plugin_url."js/jquery-datatables-1.10.20/datatables.min.js",array( 'jquery' ),EME_VERSION);
   ...
   wp_register_style(  'eme-leaflet-css', $eme_plugin_url.'js/leaflet-1.7.1/leaflet.css');
   wp_register_script( 'eme-leaflet-maps', $eme_plugin_url.'js/leaflet-1.7.1/leaflet.js', array('jquery'), EME_VERSION, true);
...
function eme_register_scripts() {
   ...
   if (get_option('eme_map_is_active'))
      wp_enqueue_style(  'eme-leaflet-css',$eme_plugin_url.'js/leaflet-1.7.1/leaflet.css');
   wp_register_script( 'eme-leaflet-maps', $eme_plugin_url.'js/leaflet-1.7.1/leaflet.js', array('jquery'), EME_VERSION, true);
